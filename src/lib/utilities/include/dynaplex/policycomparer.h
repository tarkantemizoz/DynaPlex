...
namespace DynaPlex::Utilities {
	class PolicyComparer {
		...
		template<bool SkipTrivialActions=true>
	...
	public:
		 ...
		 * 
		 * @param compute_gap Whether to compute the performance gap (in addition to the absolute performance) of the target policy regarding the benchmark policy.
		 */
		std::vector<VarGroup> Compare(DynaPlex::Policy first, DynaPlex::Policy second, int64_t index_of_benchmark = -1, bool compute_gap = false, bool rewardProblem = false) const;
		/**
		 ...
		 * 
		 * @param compute_gap Whether to compute the performance gap (in addition to the absolute performance) of the policies regarding the benchmark policy.
		 */
