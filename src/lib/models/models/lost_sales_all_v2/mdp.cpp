...
namespace DynaPlex::Models {
	namespace lost_sales_all_v2 /*keep this in line with id below and with namespace name in header*/
	{
		...
		double MDP::ModifyStateWithEvent(State& state,const MDP::Event& event) const
		{
			...
			if (state.stochasticLeadtimes) {
				if (state.order_crossover == 1) {	
					...
					for (int64_t i = state.min_leadtime; i <= state.max_leadtime; i++) {
						...
						if (i == 0) {
							...
							for (action_num = lb; action_num < inv; action_num++) {
								if (event.second[action_num] <= state.cumulative_leadtime_probs.front()) {
									onHand++;
							...
							if (state.order_initializationPhase > 0 && state.censoredDemand && state.orders_received > orders_received) {
					...
					for (int64_t i = state.min_leadtime; i <= state.max_leadtime; i++) {
						if (random_var <= state.cumulative_leadtime_probs[i]) {
							if (i == 0) {
								received = state.state_vector.back();
								onHand += received;
								state.state_vector.back() = 0;
								if (state.censoredLeadtime && received > 0) {
									state.past_leadtimes.front()++;
									state.orders_received++;
									min_found = true;
								}
								...
								received = state.state_vector.at(max_leadtime - i);
								new_coming_orders += received;
								state.state_vector.at(max_leadtime - i) = 0;
								if (state.censoredLeadtime && !min_found && received > 0) {
									state.past_leadtimes[i]++;
									state.orders_received++;
									min_found = true;
								}
				...
				if (state.min_leadtime == 0) {
					onHand += state.state_vector.back();
					state.state_vector.back() = 0;
				}
				else {
					new_coming_orders += state.state_vector.at(max_leadtime - state.max_leadtime);
		...
		void MDP::GetFeatures(const State& state, DynaPlex::Features& features) const {
			...
			features.Add(state.order_crossover);
		...
		MDP::State MDP::GetInitialState(RNG& rng) const
		{
			...
			if (evaluate) {
				...
				if (state.min_leadtime != state.max_leadtime && order_crossover) {
					state.order_crossover = 1;
				}
				state.p = p;
				...
				state.estimated_max_leadtime = state.max_leadtime;
				state.estimated_min_leadtime = state.min_leadtime;
				if (state.min_leadtime != state.max_leadtime && rng.genUniform() < 1.0) 
					state.order_crossover = 1;
		...
		void Register(DynaPlex::Registry& registry)
		{
			DynaPlex::Erasure::MDPRegistrar<MDP>::RegisterModel(
				/*=id though which the MDP will be retrievable*/ "lost_sales_all_v2",
